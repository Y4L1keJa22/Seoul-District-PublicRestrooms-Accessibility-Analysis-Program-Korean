...
이 프로그램은 서울에서 선택한 시간과 검색한 지역에 따라 범위를 자치구 또는 행정동 구역으로 잡아서, 이 구역에 공중화장실을 분석해서 검색한 지역(지점)과 가장 가까운 5개의 공중화장실을 찾아내서, 접근성 점수/인구/부족도 등급/설치 우선도개방시간/유형/주소/거리 관련 분석 결과 및 그래프를 추출합니다  
